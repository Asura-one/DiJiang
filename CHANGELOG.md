...
- 为非平凡实现和质量检查增加明确的 scope boundary，范围外发现仅在阻塞当前交付时进入修复。

### 修复

- 在 active-task pointer 读写、task 加载/保存和文档脚手架边界统一校验 task reference，并阻止绝对路径、目录穿越和 symlink 越界。
- 远程 template 统一使用严格 manifest 路径校验和唯一 staging/backup 目录，完整下载后再替换缓存，失败时保留旧版本。
