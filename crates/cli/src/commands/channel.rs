...
pub fn cmd_channel_status(channel_id: &str) -> anyhow::Result<()> {
    ...
    if inbox_path.exists() {
        println!(
            "    inbox: {} bytes",
            std::fs::read_to_string(&inbox_path)?.len()
        );
    }
    ...
    if output_path.exists() {
        println!(
            "    output: {} bytes",
            std::fs::read_to_string(&output_path)?.len()
        );
    }
    Ok(())
}
...
pub fn cmd_channel_stop(channel_id: &str) -> anyhow::Result<()> {
    ...
    if channel_toml.exists() {
        ...
        std::fs::write(
            &channel_toml,
            content.replace("status = \"active\"", "status = \"stopped\""),
        )?;
    }
...
pub fn cmd_channel_execute_all(
    model: Option<&str>,
    provider: Option<&str>,
    timeout: u64,
) -> anyhow::Result<()> {
    ...
    let handles: Vec<_> = active_channels
        .into_iter()
        .map(|channel_id| {
            let cwd = cwd.clone();
            ...
            thread::spawn(move || {
                let result = cmd_channel_execute_single(
                    &channel_id,
                    model.as_deref(),
                    provider.as_deref(),
                    timeout,
