use crate::util::{
    git_current_branch, git_worktree_root, require_dijiang_dir, run_git, trim_required,
};
    ...
    if !git_has_head(project_root)? {
        return Ok(Some(WorktreeDecision {
            readiness: dijiang_task::worktree_readiness(
                ...
                false,
                Some(
                    "当前 git 仓库还没有提交，无法创建任务 worktree；请先建立基线提交。"
                        .to_string(),
                ),
            ),
        }));
...
pub fn cmd_dispatch(
    ...
    let existing_task = if force_new {
        None
    } else {
