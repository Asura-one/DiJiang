/// DiJiang session wrapper skill definitions — thin routing layer.
///
/// This module only describes the three `dijiang-*` session wrappers.
/// They handle session lifecycle (start/continue/finish-work) and route
/// actual work to the managed `dj-*` skill ecosystem:
///
///   dispatch → align → implement → check → finish
///
/// Managed `dj-*` core skills are embedded and synced by `dj_skills.rs`.
/// Keep wrapper lifecycle text here separate from core skill taxonomy.
