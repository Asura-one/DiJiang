...
这套 Git 规则已经有一部分下沉成 runtime gate。Phase 2 Git Gate 覆盖 dispatch / active-task implementation route 的 readiness、provision 和 blocked reason。跨 worktree 的 `.dijiang` discovery 已由 `crates/cli/src/util.rs::resolve_dijiang_dir` 统一（worktree 本地 → 同仓 sibling → legacy 上溯，兼容 `.trellis`）。`finish-work` 使用同一 discovery，并对 integrate / push / cleanup 走 Phase 4 capability approval gate（`evaluate_capability`），不是 Phase 2 Git Gate evaluator 的同一条路径。
