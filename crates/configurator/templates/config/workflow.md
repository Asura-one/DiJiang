...
7. **主仓库落地** — 任务分支完成但未能执行 `--integrate` 时，必须在主 checkout 明确执行 `git merge <task-branch>` 或记录未合并原因。任何面向本机安装、发布、演示或后续开发的命令，都必须基于已合并后的主 checkout，避免从旧代码执行 `make install`、构建或更新。
